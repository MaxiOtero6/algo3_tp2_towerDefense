...
public class BotonBlancaEventHandler implements EventHandler<ActionEvent> {
    ...
    public BotonBlancaEventHandler(BordesDefensas bordes, Jugador jugador, List<CasillaMapaEventHandler> casillas,
    ...
    public void handle(ActionEvent actionEvent){
        if (jugador.obtenerCreditosRestantes() >= 10) {
            ...
            for (CasillaMapaEventHandler casilla : casillas) {
                casilla.asignarDefensa(new TorreBlanca());
            }
        }else{
            sonidoError.play();
        }
